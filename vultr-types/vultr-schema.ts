export interface paths {
    ...
    "/cdns/push-zones/{pushzone-id}/files": {
         ...
         * @description Create a presigned post endpoint that can be used to upload a file to your Push Zone.  After sending this request you must send a second POST request to the returned URL. Include all of the returnd inputs as form-data fields using the same key and value.  You must also include a field named "file" that holds the file to be uploaded.
         */
        ...
        requestBody?: {
            content: {
                "application/json": {
                     ...
                     * @enum {string}
                     */
                    log_line_prefix?: "'pid=%p,user=%u,db=%d,app=%a,client=%h '" | "'%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '" | "'%m [%p] %q[user=%u,db=%d,app=%a] '";
